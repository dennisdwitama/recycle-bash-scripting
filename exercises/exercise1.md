...
3. Move the `countries` file to your `tradingSystem` directory. Rename the `places` file with a new name of cities. Copy the cities file to your `tradingSystem` directory 
4. Create a file called `myProgram` in your home directory. Look at the permissions on `myProgram`. Change the permissions on `myProgram` to `-rwxr-x--x` 
...
8. Copy the directories `day1` and `day2` and their contents, from `/student_files` into your 
home directory. 
